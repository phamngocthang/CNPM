              ...
              <jsp:include page="leftcontent.jsp" />
			    ...
			    <div class="panel-body">
			        <div id="search">
			            <select id="search_cn" class="form-control text-center" onchange="changeFunc();">
			                <option value="${pageContext.request.contextPath}/DanhSachDeTai?index=1&cn=-1&loai=${loai}" ${tagcn==-1 ? "selected": ""}>Tất cả chuyên ngành</option>
			                <option value="${pageContext.request.contextPath}/DanhSachDeTai?index=1&cn=1&loai=${loai}" ${tagcn==1 ? "selected": ""}>Công nghệ phần mềm</option>
			                <option value="${pageContext.request.contextPath}/DanhSachDeTai?index=1&cn=2&loai=${loai}" ${tagcn==2 ? "selected" : ""}>Hệ thống thống tin</option>
			                <option value="${pageContext.request.contextPath}/DanhSachDeTai?index=1&cn=3&loai=${loai}" ${tagcn==3 ? "selected" : ""}>Mạng máy tính</option>
			            </select>
		        ...
		        <div id="show_detai">
		            ...
		            <table class="table" data-page-size="5">
		                ...
		                <tbody>
								<c:forEach items="${listD}" var="rs">
									<c:set var="acc" value="${rs.account}" />
									<c:set var="infor" value="${acc.inforaccount}" />
									<c:set var="detai" value="${rs.detai}" />
									<c:set var="chuyennganh" value="${detai.chuyennganh}" />
									<tr>
			                            <td><span class="text-info">${rs.idDangKy}</span></td>
			                            <td><span class="caret"></span>${detai.tenDeTai}</td>
			                            <td>${infor.fullName}</td>
			                            <td>${chuyennganh.tenChuyenNganh}</td>
			                            
			                            <c:choose>
			                            	<c:when test="${rs.trNhom != '' and rs.thanhVien != ''}">
			                                	<td><a class="badge red">2</a> / <a class="badge green">2</a></td>
			                                </c:when>
			                                <c:when test="${rs.trNhom != '' or rs.thanhVien != ''}">
			                                	<td><a class="badge red">1</a> / <a class="badge green">2</a></td>
			                                </c:when>
			                                <c:otherwise>
			                                	<td><a class="badge red">0</a> / <a class="badge green">2</a></td>
			                                </c:otherwise>
			                            </c:choose> 
			                            
			                                <td><a target="_blank" href="ChiTietDeTai?id=${rs.idDangKy}" class="btn btn-success btn-xs"><img src="Content/public/images/Windows-View-Detail-icon.png" width="24" height="24" data-toggle="tooltip" title="Xem chi tiết" /></a></td>    
		                       		 </tr>
								</c:forEach>
		                </tbody>
		                <tfoot>
		                    <tr>
		                        <td colspan="6">
		                            ...
		                            <div class="pagination-container">
		                            	<ul class="pagination">
		                            		<c:if test="${tag > 1}">
		                            			<li class="PagedList-skipToPrevious"><a href="${pageContext.request.contextPath}/DanhSachDeTai?index=${tag-1}&cn=${tagcn}&loai=${loai}" rel="prev">«</a></li>
		                            		</c:if>
		                            		<c:forEach begin="1" end="${endP}" var="i">
												<c:choose>
												   ...
												   <c:otherwise>
												   		<li><a href="${pageContext.request.contextPath}/DanhSachDeTai?index=${i}&cn=${tagcn}&loai=${loai}">${i}</a></li>
												   </c:otherwise>   
												</c:choose>   
					                        </c:forEach>
					                        <c:if test="${endP != tag}">
		                            			<li class="PagedList-skipToNext"><a href="${pageContext.request.contextPath}/DanhSachDeTai?index=${tag+1}&cn=${tagcn}&loai=${loai}" rel="next">»</a></li>
		                            		</c:if>
		                            	</ul>
		                            </div>
		                        </td>
